...
export default function Week2() {
  ...
  useEffect(() => {
    timer.current = setInterval(() => {
      time > 0 ? setTime(time => time - 1): setTime(0);
      console.log('1초마다 실행');
    }, 1000);
  ...
  const handleRegister = (e) => {
    ...
    timetable.forEach(subject => subject.name === e.target.name ? alert("(" + subject.title + ")" + "수강신청에 성공하였습니다!"): subject);
  }
      ...
      <table border={"1"}>
        <tbody>
          ...
          {timetable.map(subject => <tr key={subject.name}>
            ...
            <td>{subject.time}</td>
            <td><button name={subject.name} onClick={handleRegister}>신청</button></td>
            </tr>)}
