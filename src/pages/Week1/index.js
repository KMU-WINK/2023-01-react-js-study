...
export default function Week1() {
  ...
  const {bold, italic, color } = textStyle; //비구조화 할당을 통해 값 추출
  ...
  const handleCheckbox = e => {
    ...
    const { bold, italic, color } = e.target //우선 e.target에서 bold와 italic, color을 추출
    setTextStyle({
      ...textStyle, //기존의 textStyle 객체를 복사한 뒤
      ...
      <StyledH2 bold={textStyle.bold} italic={textStyle.italic} color={textStyle.color} >
        {
          ...
          Boolean(content)? [content]:'내용을 입력하세요.'
        }
      </StyledH2>
    </div>
  );
